...
COLOR_CATEGORIES = {
    ...
    "RED": {"h_ranges": [(0, 10), (170, 179)]},
    "ORANGE_BROWN": {"h_ranges": [(11, 25)]}, # Combined Orange and Brown
...
class YoloPoseTrackerColorCategoriesEfficient: # Renamed class slightly
    ...
    def process_frame(self, frame: np.ndarray) -> List[Tuple[int, int, int, int, str, Tuple[int, int, int]]]:
        ...
        # --- 1. YOLO Pose Detection & Feature Extraction (Use Efficient Color Categories) ---
            ...
            for i in range(len(boxes)):
                ...
                if torso_bbox:
                    ...
                    if ty2 > ty1 and tx2 > tx1:
                        ...
                        # *** Calculate Color Category Distribution (Efficiently) ***
                        current_color_dist = self._get_color_category_distribution_efficient(torso_patch) # <-- USE NEW FUNCTION
                    else: torso_patch = None

                person_info[i] = {
                    "person_bbox": person_bbox, "face_bbox": face_bbox, "torso_bbox": torso_bbox,
                    "face_idx_ds": face_idx_ds,
                    "current_color_dist": current_color_dist, # Store distribution
        ...
        # --- 3. Gather Matching Evidence (Using Efficient Color Categories) ---
        ...
        # Update face_id_to_perm_id_map
        current_face_mapping = {}
        for perm_id, data in active_permanent_ids.items():
             linked_face_id = data.get("face_track_id")
             if linked_face_id:
                  if linked_face_id in current_face_mapping and current_face_mapping[linked_face_id] != perm_id: print(f"Warning: Face ID {linked_face_id} multi-linked.")
                  elif linked_face_id not in current_face_mapping: current_face_mapping[linked_face_id] = perm_id
        ...
        for person_idx, info in person_info.items():
            ...
            current_distribution = info.get("current_color_dist") # Get current distribution

            # --- 3a. Collect Color Matches (Category Distribution Based) ---
            if current_distribution is not None:
                 ...
                 for perm_id, track_data in active_permanent_ids.items():
                     ...
                     if stored_distribution is not None:
                         ...
                         if match_similarity >= self.color_sim_thresh:
                             reason += f" >= Thresh({self.color_sim_thresh:.2f}) -> STRONG MATCH"
                             person_candidates[person_idx]["color"].append((perm_id, match_similarity))
                         else:
                 ...
                 person_candidates[person_idx]["color"].sort(key=lambda x: x[1], reverse=True) # Sort by similarity desc
        ...
        # --- 4a. Assign based on Direct Face Link ---
        if self.enable_debug_draw: print("--- Assignment Step 4a (Face Links) ---")
        for person_idx in persons_to_process:
             linked_perm_id = person_candidates[person_idx]["face_perm_id"]
             if linked_perm_id:
                 if linked_perm_id in active_permanent_ids:
                     if linked_perm_id not in assigned_perm_ids:
                         ...
                         if self.enable_debug_draw: print(f"  P{person_idx} assigned {linked_perm_id} via face.")
                     else: assigned_person_indices.add(person_idx);
                 else: assigned_person_indices.add(person_idx)

        # --- 4b. Assign based on Color Matches ---
        if self.enable_debug_draw: print("--- Assignment Step 4b (Color Dist Score Priority) ---")
        color_candidates = []
        for person_idx in persons_to_process:
            if person_idx not in assigned_person_indices:
                strong_color_matches = person_candidates[person_idx]["color"]
                for perm_id, score in strong_color_matches:
                    if perm_id in active_permanent_ids and perm_id not in assigned_perm_ids:
                        if score >= self.color_sim_thresh:
                            color_candidates.append({"person_idx": person_idx, "perm_id": perm_id, "score": score})
        ...
        for candidate in color_candidates:
            ...
            if person_idx not in assigned_person_indices and perm_id not in assigned_perm_ids:
                ...
                assigned_person_indices.add(person_idx); assigned_perm_ids.add(perm_id)
                if self.enable_debug_draw: print(f"  P{person_idx} assigned {perm_id} via color (Dist Sim: {candidate['score']:.3f}).")
        ...
        for person_idx, perm_id in assignments.items():
              if perm_id in self.permanent_tracks:
                  ...
                  current_face_id = info.get("current_face_track_id")
                  if current_face_id and self.permanent_tracks[perm_id].get("face_track_id") != current_face_id:
        ...
        for person_idx in persons_to_process:
             if person_idx not in assigned_person_indices:
                 ...
                 if current_face_id and current_face_id in self.face_id_to_perm_id_map:
                     ...
                     can_create_new_id = False; reason = f"Face ID {current_face_id} linked to {existing_perm_id}"
                     if existing_perm_id not in assigned_perm_ids and existing_perm_id in active_permanent_ids:
                          assignments[person_idx] = existing_perm_id; assigned_person_indices.add(person_idx); assigned_perm_ids.add(existing_perm_id)
                          self.permanent_tracks[existing_perm_id]["last_seen_frame"] = self.frame_count; self.permanent_tracks[existing_perm_id]["bbox"] = info["person_bbox"]
                          self.last_assignments = assignments
                 ...
                 if can_create_new_id:
                      ...
                      for track_data in self.permanent_tracks.values():
                           ...
                           if locked_dist is not None:
                               ...
                               if intersection > self.color_dist_thresh:
                                   is_distinct_dist = False; break
                      if not is_distinct_dist:
                           can_create_new_id = False
                           reason = f"Color dist too similar to existing (max intersection: {max_intersection:.3f} > {self.color_dist_thresh:.3f})"
                 ...
                 if can_create_new_id:
                     ...
                     self.permanent_tracks[new_perm_id] = {
                         ...
                         "locked_color_dist": current_distribution, # Store distribution
                         "vis_color": new_vis_color,
                         "last_seen_frame": self.frame_count,
                         "bbox": info["person_bbox"]
                     }
                     if current_face_id:
                         if current_face_id in self.face_id_to_perm_id_map: print(f"Warning: Overwriting face map for {current_face_id}")
                         self.face_id_to_perm_id_map[current_face_id] = new_perm_id

                     assignments[person_idx] = new_perm_id
                     assigned_person_indices.add(person_idx); assigned_perm_ids.add(new_perm_id)
                     self.last_assignments = assignments
    ...
    def draw_tracks(self, frame: np.ndarray, tracks: List[Tuple[int, int, int, int, str, Tuple[int,int,int]]]) -> np.ndarray:
        ...
        for (x1, y1, x2, y2, permanent_id, vis_color) in tracks:
            p_idx = -1; linked_face_id = "N/A"
            locked_dist = self.permanent_tracks.get(permanent_id, {}).get("locked_color_dist")
            dom_cat = "N/A"
            ...
            for idx, assigned_id in assignments.items():
                 if assigned_id == permanent_id and idx in person_info:
                    p_idx = idx
                    linked_face_id = person_info[idx].get("current_face_track_id", "N/A")
                    break

            label = f"ID:{permanent_id}"
            if linked_face_id != "N/A": label += f" F:{linked_face_id}"
            if dom_cat != "N/A": label += f" C:{dom_cat}" # Add dominant color category

            cv2.rectangle(frame, (x1, y1), (x2, y2), vis_color, 2)
            (w, h), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
            cv2.rectangle(frame, (x1, y1 - h - 15), (x1 + w + 5 , y1 - 5), vis_color, -1)
            ...
            if DRAW_DEBUG_BOXES and p_idx != -1:
                 ...
                 if info.get("torso_bbox"):
                     ...
                     # Show current dominant category if available
                     ...
                     torso_label = f"T Cur:{curr_dom_cat}"
                     if locked_dist: torso_label += f" Lock:{dom_cat}"
...
def main():
    ...
    # Face
    FACE_MAX_IOU_DIST=0.9; FACE_MAX_AGE=250; FACE_N_INIT=20; FACE_MAX_COS_DIST=0.35; FACE_NN_BUDGET=1000
    # Color Category Params - NEED TUNING
    COLOR_SIM_THRESH = 0.5   # Min intersection for match (0.0-1.0)
    COLOR_DIST_THRESH = 0.5 # Max intersection for distinctness (0.0-1.0). Higher = harder to be distinct.
    # General
    ...
    while True:
        ...
        try:
            ...
            cv2.imshow("YOLO Pose + Permanent Tracker (Efficient Color Categories)", annotated_frame)
